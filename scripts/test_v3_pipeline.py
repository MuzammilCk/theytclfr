...
def submit_job(url: str):
    ...
    if response.status_code == 404:
        print("V3 route not found, falling back to v1")
...
def get_results(job_id: str):
    response = httpx.get(f"{API_URL}/results/{job_id}?view=FULL", headers=get_auth_headers(), timeout=10.0)
    if response.status_code == 404:
        # Fallback to V1 result endpoint format if v3 doesn't exist
...
def run_test():
    test_video = "https://www.youtube.com/watch?v=PLlaomWufTM" # the one from user log
