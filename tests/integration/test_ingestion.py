...
if not TEST_DATABASE_URL:
    pytest.skip("TEST_DATABASE_URL environment variable is not set", allow_module_level=True)
...
def test_submit_job_creates_db_record(mock_delay, db_session):
    response = client.post("/api/v1/jobs", json={"youtube_url": "https://www.youtube.com/watch?v=dQw4w9WgXcW"})
...
def test_job_status_returns_pending(db_session):
    job = Job(youtube_url="https://www.youtube.com/watch?v=abcdefghijk", status="pending")
    db_session.add(job)
    db_session.commit()
