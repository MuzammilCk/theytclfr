...
class EvidenceGraphORM(Base):
    """SQLAlchemy ORM for evidence_graphs table."""
    ...
    dominant_subject: Mapped[str | None] = mapped_column(
        Text, nullable=True
    )
    groq_summary: Mapped[str | None] = mapped_column(
        Text, nullable=True
    )
    scene_boundaries_json: Mapped[dict] = mapped_column(
        JSON, nullable=False
    )
