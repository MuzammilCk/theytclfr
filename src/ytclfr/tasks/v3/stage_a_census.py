"""Celery task for V3 Stage A — Signal Census.
...
V3 variant: uses ``ytclfr.contracts.v3.manifest.SignalManifest``,
sets ``v3_stage_a_*`` job statuses, and chains to
``v3_run_targeted_extraction``.
"""
...
@celery_app.task(  # type: ignore[misc]
    bind=True,
    name="ytclfr.tasks.v3.stage_a_census.v3_run_signal_census",
    queue="heavy",
    max_retries=2,
    default_retry_delay=30,
)
def v3_run_signal_census(
    self: object, job_id: str
) -> dict[str, object]:
    """Orchestrate V3 Stage A: probe audio, visual, and metadata signals.

    Produces and persists a ``SignalManifest`` (V3 contract) for the given
    job.  Emits SSE events at start, per-probe completion, and on finish.
    This task is idempotent: if a manifest already exists for this
    job_id, it overwrites it.
    ...
    with db_session() as session:
        ...
        try:
            # Step 3 — Fetch job record and mark running
            job = (
                session.query(Job)
            ...
            if metadata_result and metadata_result.metadata_structural_hints:
                ...
                if hints.get("title_has_ordinals") or hints.get(
                    "title_has_list_keywords"
                ):
                    metadata_prior = 0.7
            ...
            # Step 8 — Merge all probes into V3 SignalManifest
            ...
            orm_manifest = _manifest_store.create(session, manifest)
            ...
            # Step 11 — Chain to V3 Stage B
            from ytclfr.tasks.v3.stage_b_extraction import (
                v3_run_targeted_extraction,
            )

            v3_run_targeted_extraction.delay(job_id)
            logger.info(
                "V3 Stage B triggered for job %s", job_id
            )
            ...
            raise self.retry(exc=exc, countdown=30)  # type: ignore[union-attr]
        finally:
