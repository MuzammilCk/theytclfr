"""Celery task for Stage B — Targeted Extraction.
...
were confirmed by probing. The chord callback is build_timeline
(V1 temporary — see STAGE-C-TODO).
...
def _build_extractor_names(manifest: SignalManifest) -> list[str]:
    ...
    Used by run_targeted_extraction and testable in isolation.
...
@celery_app.task(  # type: ignore
    bind=True,
    name="ytclfr.tasks.stage_b.run_targeted_extraction",
    queue="fast",
    max_retries=2,
...
def run_targeted_extraction(
    ...
    Stage B of the V2 evidence-based pipeline. Reads the manifest
    ...
    This task is idempotent: if the job is already in a post-Stage-B
    status (extracting, aligning, aligned, completed), it returns
    immediately without re-firing the extractor group.
    ...
    try:
        ...
        with db_session() as session:
            ...
            if job.status in POST_STAGE_B_STATUSES:
                logger.warning(
                    "Stage B idempotency hit for job %s — "
            ...
            # Step 4 — Update job status to stage_b_running
            job.status = "stage_b_running"
            session.commit()
